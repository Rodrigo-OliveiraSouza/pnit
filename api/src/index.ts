...
function getPasswordResetTtlMinutes(env: Env) {
  const parsed = env.PASSWORD_RESET_TTL_MINUTES
    ? Number(env.PASSWORD_RESET_TTL_MINUTES)
    : NaN;
...
async function sendResendEmail(env: Env, payload: EmailPayload) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Erro no Resend ${response.status}: ${message}`);
  }
}
...
app.post("/auth/password/reset/request", async (c) => {
  ...
  const body = (await c.req.json().catch(() => null)) as
    ...
    | null;
  if (!body?.email) {
    return jsonError(c, 400, "email e obrigatorio");
  }
  ...
  await sql(
    ...
    INSERT INTO password_reset_codes (
      user_id, code_hash, code_salt, expires_at,
  ...
  const resetUrl = baseUrl ? `${baseUrl}/recuperar-senha` : "";
  const subject = "Codigo de recuperacao de senha";
  const text = [
    `Seu codigo de recuperacao de senha: ${code}.`,
    `Ele expira em ${ttlMinutes} minutos.`,
    resetUrl ? `Use em: ${resetUrl}` : "",
  ]
    .filter(Boolean)
    .join("\n\n");
  const html = `
    <p>Seu codigo de recuperacao de senha:</p>
    <p><strong>${code}</strong></p>
    <p>Ele expira em ${ttlMinutes} minutos.</p>
    ${resetUrl ? `<p>Use em: <a href="${resetUrl}">${resetUrl}</a></p>` : ""}
  `;
  try {
    ...
    return jsonError(c, 502, "Falha ao enviar email", "EMAIL_FAILED");
  }
  return c.json({ ok: true });
});
...
app.post("/auth/password/reset/confirm", async (c) => {
  ...
  if (!body?.email || !body.code || !body.password) {
    return jsonError(c, 400, "email, codigo e senha sao obrigatorios");
  }
  const email = body.email.toLowerCase().trim();
  if (!email) {
    return jsonError(c, 400, "email, codigo e senha sao obrigatorios");
  ...
  if (users.length === 0) {
    return jsonError(c, 400, "Codigo invalido ou expirado", "INVALID_CODE");
  }
  ...
  let matched: { id: string; code_hash: string; code_salt: string } | null =
    null;
  for (const record of codes) {
    ...
    const hashed = await hashPassword(codeValue, row.code_salt);
    if (hashed.hash === row.code_hash) {
      matched = row;
      break;
    }
  }
